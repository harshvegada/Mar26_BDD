...
public class OrderTrackSummaryPage extends CommonPage {
    ...
    private final String DELIVER_STATUS = "//div[@data-testid='track-step-delivered']/div[2]";
    private final String ENJOY_MEAL_TEXT = "//span[contains(text(),'Enjoy your meal!')]";
...
//        String className = driver.findElement(By.xpath(DELIVER_STATUS)).getAttribute("class");
        ...
        if (className.contains("font-semibold text-slate-700")) {
            return true;
        } else if (className.contains("font-semibold text-slate-400")) {
            return false;
        }
        return false;
    }
